...
namespace BANKING_APPLICATION.Implementation.Account_Implementation
{
    public class CreateAccount : ICreateAccount
    {
        ...
        public void CreateNewAccount(Customer UserLoggedIn)
        {
            Console.Clear();
            ...
            void SaveCreatedAccDetails()
            {
                ...
                Account myAccount = new Account(UserLoggedIn.Id, UserLoggedIn.Name, AcNo, AcType, Bal);

                using (StreamWriter writer = new StreamWriter("Accounts.txt", true))
                {
                    writer.WriteLine($"| {myAccount.Id}   |   {myAccount.Name}   |   {myAccount.AccountNumber}   |  {myAccount.AccountType}   |  {myAccount.Balance}  | \n\n");
                }
